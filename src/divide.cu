...
__global__ void divide_kernel(int X, int Y, int Z, int C, float const *in, float *outn, float *outs, float *oute, float *outw, float *outf, float *outb) {
	...
	copy_c(in + in_index, outn + get_index(X, Y, Z, C, x, y,         z), X * Y, X * Y, C);
	copy_c(in + in_index, outs + get_index(X, Y, Z, C, x, y, Z - 1 - z), X * Y, X * Y, C);

	copy_c(in + in_index, oute + get_index(Z, Y, X, C, z, y,         x), X * Y, Z * Y, C);
	copy_c(in + in_index, outw + get_index(Z, Y, X, C, z, y, X - 1 - x), X * Y, Z * Y, C);

	copy_c(in + in_index, outf + get_index(X, Z, Y, C, x, z,         y), X * Y, X * Z, C);
	copy_c(in + in_index, outb + get_index(X, Z, Y, C, x, z, Y - 1 - y), X * Y, X * Z, C);
}
...
__global__ void combine_kernel(int X, int Y, int Z, int C, float *in, float const *outn, float const *outs, float const *oute, float const *outw, float const *outf, float const *outb) {
	...
	int in_index = get_index(X, Y, Z, C, x, y, z);
	add_c(outn + get_index(X, Y, Z, C, x, y,         z), in + in_index, X * Y, X * Y, C);
	add_c(outs + get_index(X, Y, Z, C, x, y, Z - 1 - z), in + in_index, X * Y, X * Y, C);

	add_c(oute + get_index(Z, Y, X, C, z, y,         x), in + in_index, Z * Y, X * Y, C);
	add_c(outw + get_index(Z, Y, X, C, z, y, X - 1 - x), in + in_index, Z * Y, X * Y, C);

	add_c(outf + get_index(X, Z, Y, C, x, z,         y), in + in_index, X * Z, X * Y, C);
	add_c(outb + get_index(X, Z, Y, C, x, z, Y - 1 - y), in + in_index, X * Z, X * Y, C);
